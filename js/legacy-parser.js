function legacyParseAlternate(summary, description) {
    ...
    for (let i = 0; i < periodItems.length; i++) {
      ...
      if (matches) {
        ...
        if (foundDuplicate) {
          ...
          else if (~periodName.indexOf("flextime")
              || ~periodName.indexOf("self")
              || ~periodName.indexOf("assembly")
              || ~periodName.indexOf("tutorial"))
            periodName = "flex";
        ...
        periods[periods.length - 1].name += periodItems[i].trim();
      }
    }
    return periods;
